...
describe("sandboxIdFromTrace", () => {
  it("reads curie.sandbox_id from trace metadata", () => {
    expect(sandboxIdFromTrace({ metadata: { "curie.sandbox_id": "runner-deal-desk-abc123" } })).toBe(
  ...
  it("reads it from OTel resource attributes", () => {
    expect(
      sandboxIdFromTrace({ resourceAttributes: { attributes: { "curie.sandbox_id": "sbx-9" } } }),
    ).toBe("sbx-9");
  });
  ...
  it("prefers a top-level typed sandbox_id (the API's hoisted field)", () => {
    ...
    expect(
      sandboxIdFromTrace({ sandbox_id: "sbx-typed", trace: { metadata: { "curie.sandbox_id": "sbx-probed" } } }),
    ).toBe("sbx-typed");
  });

  it("falls back to probing the nested trace when the typed field is absent", () => {
    expect(sandboxIdFromTrace({ sandbox_id: null, trace: { metadata: { "curie.sandbox_id": "sbx-probed" } } })).toBe(
      "sbx-probed",
    );
  });
  ...
  it("returns null when absent or blank, so the UI degrades silently", () => {
    ...
    expect(sandboxIdFromTrace({ metadata: { "curie.sandbox_id": "" } })).toBeNull();
