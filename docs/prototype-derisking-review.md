...
Author: prototype-derisking pass, 2026-07-04. Companion to the internal strategy notes (the product-direction and architecture-decision-menu writeups) and the build plan for AgentOS in `reference/detailed-architecture.md`. This document exists to answer one question: **which architecture assumptions in the AgentOS build plan are most likely to be wrong, and which of those can a cheap mini-prototype settle before the full build starts.**
...
> **What actually got run (2026-07-04, two passes).** Pass 1: inspected the real agent-sandbox v0.5.0 CRD schema, then ran **PT-1 live on a disposable throwaway k3s cluster**. Pass 2 (after a maintainer authorized the OAuth token, gVisor install, and a capped ClickHouse run): **PT-2 core, PT-4 full, and a gVisor/Kata install on the scratch cluster.** Headline results, all from real output:
...
> - **PT-3 (R4):** gVisor/Kata install attempted on the scratch cluster — see R4 for the verdict (the box is a 4 GB LXC with no `/dev/kvm`).
>
> **The one deliverable a maintainer named that is absent: the "Claude design file" — no such file had arrived under the project tree as of 2026-07-04; treated as an assumption, not a blocker.**
...
> **PT-1 was RUN on a disposable throwaway cluster (2026-07-04).** R1's routing sub-claim is now CONFIRMED live; R1/R2's hibernation sub-claim is CONFIRMED as a cold restart. See the updated verdicts inline below and the full run log in `../test-plans/PT-1-agent-sandbox-lifecycle.md`.
...
Ranked by value-per-hour. All target the scratch cluster or a local docker/kind, never a shared cloud account.

| ID | Prototype | Settles | Sev·Unc | Feasible | Timebox | Env | Run status (2026-07-04) |
|----|-----------|---------|---------|----------|---------|-----|-----|
| **PT-1** | Agent Sandbox lifecycle + control-endpoint reachability | R1 (part), R6-adjacent | 5·5 | High | 0.5 day | scratch cluster | **RUN** — GO w/ cold-restart caveat |
| **PT-2** | ACI round-trip: streaming steer + interrupt + gen_ai spans, incl. plugin load + cache-read | R1 (part), R2, R7, R8 | 5·5 / 4·4 | Med-High | 1-1.5 day | local (OAuth) + k8s | **RUN** — auth/steer/interrupt/cache PASS (local); **runner-in-sandbox + span export PROVEN** (PT-E on the scratch cluster) |
| **PT-D** | Warm-pool allocation (extensions): pre-warm + claim binds to warm sandbox | R1 (warm-pool) | 4·4 | High | 0.5 day | scratch cluster | **RUN — PASS** (replicas:2 pre-warmed; claims bound to warm; pool replenished) |
| **PT-E** | Runner inside a Sandbox: msg via serviceFQDN → answer → nested gen_ai spans to Langfuse | R1, R7, R3 | 5·4 | Med | 0.5 day | scratch cluster + local Langfuse | **RUN — PASS** (2 turns traced from `agentos-runner-sandbox`) |
| **PT-3** | Security boundary: egress lockdown; cross-agent secret isolation; runtimeClass enforced | R4 | 5·3 | High | 0.5 day | scratch cluster | **RUN — all claims PASS** (egress+metadata blocked w/ control, secret isolation, non-root RO-rootfs, gVisor); Kata infeasible |
...
- **The declarative-DAG / config-as-definition engine** — the strategy doc explicitly says do NOT build a generic DAG engine. Nothing to prototype; the Phase-0 investment is the `agent_definitions` config-hash stamp, which is a schema change, not an infra experiment.
...
7. **Production auth model (not the dev OAuth path).** PT-2 used a maintainer's OAuth token to prove the mechanism; that is not the shipping credential for a customer runner (personal identity, per-user rate limit, refresh/expiry). The leave-behind needs a proper API-key / Bedrock / Vertex path with per-tenant key management — straightforward, but it is real work that the OAuth shortcut did not exercise.
