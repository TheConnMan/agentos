...
- **No-Slack middle-mode deploy needed a values knob (fixed).** Added the first-class `worker.slackApiBaseUrl` value (default `""`, renders `SLACK_API_BASE_URL` on the worker only when non-empty) so a Slack-free middle-mode deploy can point the worker at an external CLI stub without editing templates; `agentos message` sets it automatically.
